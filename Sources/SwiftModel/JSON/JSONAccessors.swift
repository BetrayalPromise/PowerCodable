...
extension JSON {
    ...
    public func get<T: RawRepresentable>(`default`: T? = nil) throws -> T where T.RawValue: JSONInitializable {
        do {
            return try T(json: self)
        } catch {
            guard let `default` = `default` else { throw error }
    ...
    public func get<T: RawRepresentable & JSONInitializable>(`default`: T? = nil) throws -> T? where T.RawValue: JSONInitializable {
        do {
            return try T(json: self)
        } catch {
            if case .null = self { return nil }
...
extension JSON {
    public func get(_ field: String) throws -> JSON {
